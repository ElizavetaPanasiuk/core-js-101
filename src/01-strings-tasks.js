...
function encodeToRot13(str) {
  return str.split('').map(
    (elem) => {
      if ((elem >= 'A' && elem <= 'M') || (elem >= 'a' && elem <= 'm')) {
        return String.fromCharCode(elem.charCodeAt(0) + 13);
      } if ((elem >= 'N' && elem <= 'Z') || (elem >= 'n' && elem <= 'z')) {
        return String.fromCharCode(elem.charCodeAt(0) - 13);
      }
      return elem;
    },
...
function isString(value) {
  return typeof value === 'string' || (typeof value === 'object' && value !== null && JSON.stringify(value) === `"${value.toString()}"`);
}
